...
public class ValidationBuilder {
  ...
  private static final Set<String> ValidOrderTypes = Arrays.stream(
    OrderType.values()
  )
    .map(Enum::name)
    .collect(Collectors.toSet());

  private static final Set<String> ValidTickers = Arrays.stream(Ticker.values())
    .map(Enum::name)
    .collect(Collectors.toSet());
  ...
  public ValidationBuilder validateDouble(String attribute) {
    validations.add(body -> {
      if (!missingOrEmpty(body, attribute)) try {
        Double attributeValue = Double.parseDouble(body.getString(attribute));
        if (attributeValue <= 0) {
          return ValidationResult.fail(
            attribute + " cannot be equal to or less than 0."
          );
        }
      } catch (Exception e) {
        if (e instanceof NumberFormatException) {
          return ValidationResult.fail(
            "Validation Error : " + attribute + " Price is invalid."
          );
        }
      }
      else {
        return ValidationResult.fail(
          "Validation Error : " + attribute + " is missing or blank"
        );
      }
    ...
    validations.add(body -> {
      if (!missingOrEmpty(body, "quantity")) {
        try {
          ...
          if (quantity <= 0) {
            return ValidationResult.fail(
              "Price cannot be equal to or less than 0."
            );
          }
        } catch (Exception e) {
          if (e instanceof NumberFormatException) {
            return ValidationResult.fail(
              "Validation Error : Quantity is invalid "
            );
          }
        }
      } else {
        return ValidationResult.fail(
          "Validation Error : Quantity is missing or blank"
        );
      }
      return ValidationResult.ok();
    });
    ...
    validations.add(body -> {
      if (!missingOrEmpty(body, "userId")) {
        ...
        if (
          ...
          return ValidationResult.fail(
            "Invalid userId : no such user exists in the database"
          );
        }
        return ValidationResult.ok();
      } else {
        return ValidationResult.fail(
          "Validation Error : userId is missing or blank"
        );
      }
    });
    return this;
    ...
    validations.add(body -> {
      if (!missingOrEmpty(body, "orderId")) {
        ...
        if (
          ...
          return ValidationResult.fail(
            "Invalid orderId : no such order exists in the database"
          );
        }
      } else {
        return ValidationResult.fail(
          "Validaton Error : orderId is missing or blank"
        );
      }
      return ValidationResult.ok();
    });
    ...
    validations.add(body -> {
      if (!missingOrEmpty(body, "type")) {
        if (!ValidOrderTypes.contains(body.getString("type"))) {
          return ValidationResult.fail(
            "Invalid Order Type : Must be a BUY or SELL"
          );
        }
      } else {
        return ValidationResult.fail(
          "Validation Error : order type is missing or blank"
        );
      }
      return ValidationResult.ok();
    });
  ...
  public ValidationBuilder validateTicker() {
    validations.add(body -> {
      if (!missingOrEmpty(body, "ticker")) {
        ...
        return ValidationResult.fail(
          "Validation Error : Ticker is missing or blank"
        );
      }
      return ValidationResult.ok();
    });
  ...
  public ValidationBuilder validateOrderToCancelBelongsToUser() {
    MongoCollection<Document> activeOrderCollection =
      MongoClientConnection.getCollection("activeOrders");
    ...
    validations.add(body -> {
      ...
      if (orderDoc != null) {
        if (orderDoc.getString("userId").equals(body.getString("userId"))) {
          ...
          return ValidationResult.fail(
            "Error : The order you are trying to cancel doesnt belong to you"
          );
        }
      } else {
        return ValidationResult.fail(
          "Invalid Order Id - No such order could be found."
        );
      }
    });
    return this;
