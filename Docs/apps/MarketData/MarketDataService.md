What does it do?
    ...
    acts as the interface to the MarketData api. On instantiation, it checks if 
    we are starting up the app, if we are it makes a days worth of data, otherwise
    it checks every minute if it is currently market open. The MarketDataService
    runs with a Daemon thread as the main thread is reserved for hadnling API calls.
    Created as a Singleton for thread-safety and global access contol, so every all
    market data is coming from the same source.

When do we use it?
    Anyime we need maket data: the front end LiquidityEngine. 
