...
Where do we use it?
    Snapshot is the base unit for market data in this application, it holds a timestamp,
    a price for that specific moment in time, and the change from the previous price.
...
Methods
    Records have public 'getters' but they are named after the attribute themselves rather
    than getAttribute() like in convention.
