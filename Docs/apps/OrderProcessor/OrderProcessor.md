...
* processOrder(Order order) : This is a composite method, it adds an Order to the relevant TradeBook, evaluated for matches, ands returns the list of MatchingDetails objects expressed as JSON to the API to be used to update the DB.
* cancelOrder(Order order) : Method to cancel orders, finds the Order to be cancelled in theappropriate TradeBook and removes it, returns True if order was removed sucessfully, and False if it couldnt find the order, which would mean that the order had alredy been filled and taken off the books by the time the cancellation came in, at which point nothing further can be done.