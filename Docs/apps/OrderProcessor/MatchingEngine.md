...
* match(TradeBook tradeBook) - ArrayList<String> : Takes a TradeBook as input, evaluates the TradeBook for matches, process them and returns an ArrayListof MatchingDetails JSONs that are used to update the DB.
* processMatches() : Helper method called by the match() method: this method is responsible for updating orders in the TradeBook - adjusting quantities, creating and serialising the MatchingDetails objects, appending to the matchesFound list and removing completed orders from the TradeBook, so we dont erroneously pricess them again.
* matchPossible(buyOrders, sellOrders) : Helper method used by the match() method : This method looks at both sides of the TradeBook and checks is there is exactly one match possible, returns true while one match is possible and false when there arent any matches possible. 
